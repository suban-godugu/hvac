...
export default function O9ElectronicExpansionValveStudio() {
  ...
  return (
    <OpportunityWorkspace
      ...
      actions={
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="px-3 py-1.5 rounded border border-white/[0.08] text-slate-300 text-xs font-semibold flex items-center gap-1.5">
    ...
    >
      ...
      <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-950/40 via-cyan-950/30 to-slate-900/60 border border-emerald-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
              {data?.recommendation ?? 'NO DATA'}
            </span>
            <span className="text-xs text-slate-300 font-semibold">Technical Feasibility: {data?.technical_feasibility_pct != null ? `${data.technical_feasibility_pct}%` : 'NO DATA'}</span>
          </div>
        ...
        <div className="flex items-center gap-4 shrink-0 bg-slate-950/60 px-4 py-3 rounded-xl border border-white/[0.06]">
          ...
          <div className="text-center">
            ...
            <div className="text-xl font-bold text-cyan-300 font-mono">{data?.five_year_net_roi_pct != null ? `+${data.five_year_net_roi_pct}%` : 'NO DATA'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        ...
        <div className="kpi-tile">
          ...
          <div className="text-base font-bold text-white font-mono">
            ...
            {data?.projected_suction_pressure_psig != null && <span className="text-cyan-300"> → {data.projected_suction_pressure_psig} psi</span>}
          </div>
        </div>
        <div className="kpi-tile kpi-tile-accent">
          <div className="text-emerald-300 text-xs">6. Compressor COP</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-2xl bg-slate-900/70 border border-white/[0.08] space-y-4">
          ...
          <div className="h-56 w-full bg-slate-950/60 rounded-xl p-4 border border-white/[0.04] flex items-center justify-center">
            {timeline.length === 0 ? (
        ...
        <div className="p-5 rounded-2xl bg-slate-900/70 border border-white/[0.08] space-y-4">
          ...
          <div className="h-56 w-full bg-slate-950/60 rounded-xl p-4 border border-white/[0.04] flex flex-col justify-between">
      ...
      <div className="p-5 rounded-2xl bg-slate-900/70 border border-white/[0.08] space-y-4">
        ...
        <div className="overflow-x-auto rounded-xl border border-white/[0.08]">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/80 text-slate-400 font-medium border-b border-white/[0.08]">
              <tr>
            ...
            <tbody className="divide-y divide-white/[0.04] text-slate-300 font-mono">
              ...
              {technicalParams.map((param, idx) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="py-2.5 px-3 font-semibold text-white font-sans">{param.parameter}</td>
                  <td className="py-2.5 px-3 text-rose-300">{param.current ?? 'NO DATA'}</td>
                  <td className="py-2.5 px-3 text-emerald-300 font-bold">{param.expected}</td>
                  <td className="py-2.5 px-3 text-cyan-300">{param.difference ?? 'NO DATA'}</td>
      ...
      <div className="p-5 rounded-2xl bg-slate-900/70 border border-white/[0.08] space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 text-xs">
          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-white/[0.04] space-y-1">
            <div className="text-slate-400">Estimated Retrofit Capex</div>
            <div className="text-lg font-bold text-white font-mono">{data?.estimated_capital_cost_usd != null ? `$${Number(data.estimated_capital_cost_usd).toLocaleString()}` : 'NO DATA'}</div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-white/[0.04] space-y-1">
            ...
            <div className="text-lg font-bold text-cyan-300 font-mono">{data?.annual_maintenance_savings_usd != null ? `$${data.annual_maintenance_savings_usd} / yr` : 'NO DATA'}</div>
            <div className="text-[10px] text-slate-500">Zero power bulb failures</div>
          </div>

          <div className="kpi-tile kpi-tile-accent">
            <div className="text-emerald-300">Total Annual Benefit</div>
            <div className="text-lg font-bold text-emerald-300 font-mono">{data?.total_annual_savings_usd != null ? `$${Number(data.total_annual_savings_usd).toLocaleString()} / yr` : 'NO DATA'}</div>
