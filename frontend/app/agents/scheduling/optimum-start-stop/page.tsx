    ...
    >
        ...
        <div className="glass-card p-5 space-y-4">
          ...
          <div className="space-y-2 text-xs font-mono">
            <div className="p-2.5 rounded-lg bg-slate-900/60 border border-white/5 space-y-1">
              ...
              <div className="flex justify-between"><span>Mean Absolute Error:</span><strong className="text-cyan-300">{model.mae_minutes != null ? `${model.mae_minutes} min` : 'NO DATA'}</strong></div>
              <div className="flex justify-between"><span>Root Mean Sq Error:</span><strong className="text-slate-300">{model.rmse_minutes != null ? `${model.rmse_minutes} min` : 'NO DATA'}</strong></div>
            ...
            <div className="p-2.5 rounded-lg bg-slate-900/60 border border-white/5 space-y-1">
              ...
              <div className="flex justify-between"><span>Pull-Down Rate:</span><strong className="text-slate-200">{model.parameters?.pull_down_rate || 'NO DATA'}</strong></div>
              <div className="flex justify-between"><span>Weather Sensitivity:</span><strong className="text-slate-200">{model.parameters?.weather_sensitivity || 'NO DATA'}</strong></div>
              <div className="flex justify-between"><span>Thermal Time Constant:</span><strong className="text-slate-200">{model.parameters?.thermal_time_constant || 'NO DATA'}</strong></div>
              ...
              <div className="flex justify-between"><span>Balance Point Temp:</span><strong className="text-slate-200">{model.parameters?.balance_point_temp || 'NO DATA'}</strong></div>
            </div>
          </div>
          )}
        </div>
            ...
            <table className="bms-table">
              ...
              <tbody className="font-mono text-xs">
                {(startCandidatesData || []).length === 0 ? (
                  ...
                  <tr key={i} className={String(c.decision || '').includes('SELECTED') ? 'bg-cyan-950/30' : ''}>
                    <td className="font-bold text-white text-sm">{c.candidate_time}</td>
                    <td className="text-cyan-300 font-semibold">{c.predicted_target}</td>
                    <td className="text-slate-300">{c.pulldown_min} min</td>
                    <td className="text-slate-200">{c.energy_kwh} kWh</td>
                    <td>
                      <span
            ...
            <table className="bms-table">
              ...
              <tbody className="font-mono text-xs">
                {(coastCandidatesData || []).length === 0 ? (
                  ...
                  <tr key={i} className={String(c.decision || '').includes('SELECTED') ? 'bg-cyan-950/30' : ''}>
                    <td className="font-bold text-white text-sm">{c.candidate_time}</td>
                    <td className="text-cyan-300 font-semibold">{c.expected_temp_1800 != null ? `${c.expected_temp_1800}°C` : 'NO DATA'}</td>
          ...
          <div className="space-y-3 font-mono text-xs">
            ...
            <div className="p-3 rounded-xl bg-slate-900/60 border border-white/5 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-white font-bold font-sans">Optimum Start Action:</span>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[11px] py-1 border-y border-white/5">
                ...
                <div><span className="text-[9px] text-slate-500 block">TARGET REACH</span><strong className="text-cyan-300">{startDecision.predicted_target_reached || 'NO DATA'}</strong></div>
              </div>
            ...
            <div className="p-3 rounded-xl bg-slate-900/60 border border-white/5 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-white font-bold font-sans">Optimum Coast Action:</span>
                <span className="px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-400 text-emerald-300 text-[10px] font-bold">
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[11px] py-1 border-y border-white/5">
                ...
                <div><span className="text-[9px] text-slate-500 block">TEMP @ 18:00</span><strong className="text-cyan-300">{coastDecision.predicted_temp_1800 || 'NO DATA'}</strong></div>
              </div>
              <p className="text-[11px] text-slate-400 font-sans leading-relaxed">
        ...
        <div className="glass-card p-5 space-y-4">
          ...
          <div className="space-y-2 font-mono text-xs max-h-72 overflow-y-auto pr-1">
            {(timelineData || []).length === 0 ? (
              ...
              <div key={i} className="p-2.5 rounded-lg bg-slate-900/60 border border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-cyan-400 font-bold w-12">{tl.time}</span>
                  <div>
                    <span className="text-slate-200 font-sans font-medium block">{tl.event}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="glass-card p-5 space-y-4">
          ...
          <div className="overflow-y-auto max-h-56 border border-white/5 rounded-lg">
            <table className="bms-table">
              <thead>
                <tr>
                  <th>Safety Check</th>
        ...
        <div className="glass-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 font-mono text-xs text-center p-3 rounded-lg bg-slate-900/60 border border-white/5">
            <div>
              ...
              <span className="text-white font-bold truncate block">{bmsActionData?.target_equipment || 'NO DATA'}</span>
            </div>
            <div>
            ...
            <tbody className="font-mono text-xs">
              {(historyData || []).length === 0 ? (
                ...
                <tr key={i}>
                  ...
                  <td className="text-slate-300">{h.oat}</td>
                  <td className="text-slate-300">{h.initial_temp}</td>
                  <td className="text-cyan-300 font-bold">{h.target_temp}</td>
                  <td className="text-slate-500">{h.scheduled_start}</td>
                  <td className="text-emerald-400 font-bold">{h.optimized_start}</td>
                  <td className="text-slate-200">{h.actual_start}</td>
                  <td className="text-cyan-300 font-semibold">{h.target_reached}</td>
                  <td className="text-slate-300">{h.pulldown_duration}</td>
        ...
        <div className="glass-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 font-mono text-xs">
            <div className="p-3 rounded-lg bg-slate-900/60 border border-white/5 space-y-1.5">
              ...
              <div className="flex justify-between"><span>Baseline Runtime:</span><strong className="text-slate-300">{energyData.baseline_runtime_hours} hrs</strong></div>
              ...
              <div className="flex justify-between pt-1 border-t border-white/5 font-bold"><span>Runtime Saved:</span><span className="text-emerald-400">{energyData.runtime_reduction_hours} hrs ({energyData.runtime_reduction_minutes} min)</span></div>
            </div>

            <div className="p-3 rounded-lg bg-slate-900/60 border border-white/5 space-y-1.5">
              <span className="text-[10px] text-slate-500 block font-sans">ENERGY METRICS</span>
              <div className="flex justify-between"><span>Baseline Energy:</span><strong className="text-slate-300">{energyData.baseline_energy_kwh} kWh</strong></div>
              <div className="flex justify-between"><span>Optimized Energy:</span><strong className="text-cyan-300">{energyData.optimized_energy_kwh} kWh</strong></div>
              <div className="flex justify-between pt-1 border-t border-white/5 font-bold"><span>Energy Saved:</span><span className="text-emerald-400">{energyData.daily_energy_savings_kwh} kWh / day</span></div>
            </div>
          </div>

          <div className="p-3 rounded-lg bg-slate-950/70 border border-white/5 space-y-1 text-xs font-mono">
            <div className="flex justify-between text-[11px]"><span className="text-slate-400">Predicted Savings:</span><span className="text-slate-300">{energyData.tiers?.predicted_savings_kwh != null ? `${energyData.tiers.predicted_savings_kwh} kWh / day` : 'NO DATA'}</span></div>
            <div className="flex justify-between text-[11px]"><span className="text-slate-400">Applied Savings:</span><span className="text-slate-300">{energyData.tiers?.applied_savings_kwh != null ? `${energyData.tiers.applied_savings_kwh} kWh / day` : 'NO DATA'}</span></div>
            <div className="flex justify-between text-xs font-bold pt-1 border-t border-white/5"><span className="text-slate-200">Verified Savings:</span><span className="text-emerald-400">{verifiedKwh != null ? `+${verifiedKwh} kWh / day ($${verifiedCost ?? '—'} / day)` : 'NOT VERIFIED'}</span></div>
          </div>
          </>
          )}
        </div>
