...
export function O15SafetyPanel({ data }: { data: O15Dashboard }) {
  ...
  return (
    <section className="kpi-tile" aria-labelledby="o15-safety">
      <h2 id="o15-safety" className="text-sm font-semibold text-white mb-3">
      ...
      {checks.length ? (
        <ul className="space-y-1.5">
          {checks.map((c) => {
            ...
            return (
              <li key={c.check_name} className="flex justify-between gap-3 text-xs font-mono border-b border-white/[0.04] py-1">
      ...
      <div className="mt-3 text-sm font-semibold text-white">{fmtDash(data.safety?.overall || data.overall_safety)}</div>
    </section>
  );
}
