  ...
  return (
    <section className="kpi-tile space-y-3 col-span-12" aria-labelledby="o16-safe">
      <div className="flex items-center justify-between gap-2">
        <h2 id="o16-safe" className="text-sm font-semibold text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2 text-xs font-mono">
        <div className="border border-white/10 p-2">
          ...
          <div className="text-slate-100 mt-1">{fmtDash(cfg.min_head_pressure)}</div>
        </div>
        <div className="border border-white/10 p-2">
          <div className="text-slate-500">Maximum head pressure</div>
          <div className="text-slate-100 mt-1">{fmtDash(cfg.max_head_pressure)}</div>
        </div>
        <div className="border border-white/10 p-2">
          <div className="text-slate-500">Maximum rate of change</div>
          <div className="text-slate-100 mt-1">{fmtUnit(cfg.max_pump_step_pct, '% / step')}</div>
        </div>
        <div className="border border-white/10 p-2">
          <div className="text-slate-500">Minimum condenser water flow</div>
          <div className="text-slate-100 mt-1">{fmtDash(cfg.min_cw_flow)}</div>
        </div>
        <div className="border border-white/10 p-2">
          <div className="text-slate-500">Maximum condenser water temperature</div>
          <div className="text-slate-100 mt-1">{fmtUnit(cfg.max_condensing_temp_c, '°C')}</div>
        </div>
        <div className="border border-white/10 p-2">
          <div className="text-slate-500">Compressor envelope</div>
          <div className="text-slate-100 mt-1">{compressor ? compressor.result : '—'}</div>
        </div>
      </div>
      {hold && (
      ...
      {checks.length ? (
        <ul className="space-y-1 text-xs font-mono">
          {checks.map((c) => (
            <li key={c.check_name} className="flex justify-between gap-3 border-b border-white/[0.04] py-1">
              <span className="text-slate-400">{c.check_name}</span>
              <span className={c.result === 'PASS' ? 'text-emerald-400' : 'text-amber-300'}>
