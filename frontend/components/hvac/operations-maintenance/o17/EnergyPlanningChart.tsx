...
function SnapshotTooltip({ active, payload, label }: TipProps) {
  ...
  return (
    <div className="bg-[#0c1220] border border-white/[0.08] px-2.5 py-2 text-[11px] font-mono">
      ...
      {payload.map((p) => (
        <div key={p.name} className="text-slate-200">
          {p.name}: <span className="text-cyan-300">{p.value == null ? '—' : p.value}</span> kW
        </div>
      ))}
    </div>
  );
          ...
          {(['24H', '7D', '30D', '90D'] as const).map((r) => (
            <button
              ...
              className={`px-2 py-1 text-[11px] font-mono border focus-visible:ring-2 focus-visible:ring-cyan-400 ${
                range === r ? 'border-cyan-500/40 text-cyan-300' : 'border-white/10 text-slate-400'
              }`}
              onClick={() => setRange(r)}
            >
              {r}
