  ...
  return (
    <article className="kpi-tile space-y-3">
      <div className="flex justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-100">{formatDash(issue.issueType || data.recommendation?.action)}</h3>
      ...
      <dl className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[12px]">
        <div>
          ...
          <dd className="text-slate-300 mt-0.5">{formatDash(issue.finding)}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="font-mono text-slate-200 mt-0.5">{formatKw(issue.energyImpactKw ?? data.energy?.impactKw)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Confidence</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{o19ConfidencePct(data.confidence)}</dd>
        </div>
      </dl>
      <button
        type="button"
