  ...
  return (
    <article className="kpi-tile space-y-3">
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-sm font-semibold text-slate-100">{formatDash(item.title)}</h3>
      ...
      <dl className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[12px]">
        <div>
          ...
          <dd className="text-slate-300 mt-0.5">{formatDash(item.reason)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Affected Users</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatDash(o18Affected(data))}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Required Action</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatDash(data.recommendation?.action)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Expected Awareness Impact</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatPercent(o18Coverage(data))}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Energy Impact</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatKwh(o18EnergyImpact(data), true)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Priority</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatDash(data.recommendation?.priority || data.priority)}</dd>
        </div>
      </dl>
      <button
        type="button"
