  ...
  return (
    <section className="kpi-tile space-y-3" aria-label="Control recommendation">
      <div className="flex justify-between gap-2">
        <h2 className="text-sm font-semibold text-slate-100">{formatDash(data.recommendation.action)}</h2>
      ...
      <dl className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[12px]">
        <div>
          ...
          <dd className="font-mono text-slate-200 mt-0.5">—</dd>
        </div>
        <div>
          <dt className="text-slate-500">Affected Equipment</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{o20ControllerField(data, 'controller_id')}</dd>
        </div>
        ...
        <div>
          ...
          <dd className="font-mono text-slate-200 mt-0.5">{(data.recommendation.evidence || []).filter(Boolean).join(' · ') || '—'}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Risk</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatDash(data.metrics?.change_risk)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">Priority</dt>
          <dd className="font-mono text-slate-200 mt-0.5">{formatDash(data.priority)}</dd>
        </div>
        <div>
      ...
      <button type="button" className="text-[11px] font-mono text-cyan-300 underline-offset-2 hover:underline focus-visible:ring-2 focus-visible:ring-cyan-400" aria-expanded={open} onClick={() => setOpen((v) => !v)}>
